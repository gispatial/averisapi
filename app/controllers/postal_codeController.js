...
exports.listAll = function(req, res) {
	helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  Postal_Code.getAll(req,function(err, postal_code) {

  if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",postal_code));
	}
...
helper.checkPermission(req,"v",function (isPermited) {
        if(isPermited){
  Postal_Code.getById(req,req.params.id, function(err, postal_code) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record found",postal_code));
...
helper.checkPermission(req,"u",function (isPermited) {
        if(isPermited){
  Postal_Code.updateById(req,req.params.id, new Postal_Code(req,req.body), function(err, postal_code) {
     if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
    res.status(200).send(helper.createResponse(helper.Success,1,"Record Updated",postal_code));
	}
  });
  } else{
            res.status(403).send(helper.createResponse(helper.Error,0,helper.authError,""));
...
helper.checkPermission(req,"d",function (isPermited) {
        if(isPermited){
  Postal_Code.remove(req,req.params.id, function(err, postal_code) {
      if (err){
      res.status(200).send(helper.createResponse(helper.Error,0,err,""));
	  }else{
